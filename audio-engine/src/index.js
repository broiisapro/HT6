...
async function main() {
  ...
  // melody/timbre.
  const { sourceNode, filterNode, pannerNode, lfo, playBeat } = await startPlayback();
  ...
  const { setOppositeMood, setStaticMode } = startServer({ sourceNode, filterNode, pannerNode, lfo, fallbackPlayer, playBeat });
